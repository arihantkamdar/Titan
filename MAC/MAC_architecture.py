...
class MAC(nn.Module):
    ...
    def forward(self, x, target = None,test = False):
        ...
        for t in range(num_segments):
            ...
            # I commented this rather than removing this so you can understand my thought process
    ...
    def update_memory(self, x):
        ...
        """
        # a very vauge implementation of memory writing
        self.memory_optimizer.zero_grad()
    ...
    def eval_with_adaptation(self, x, target=None, adaptation_steps=10):
        ...
        """
        self.eval()
        # I cloned the original memory params # after processing chunks i will make update the learned memory params with this one
        memory_clone = copy.deepcopy(self.memory)
...
def train(model, x, target, num_epochs=10):
    ...
    # A very simple training module with nothign fance, just total loss is combination of loss memory and task loss
